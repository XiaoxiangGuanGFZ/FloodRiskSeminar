...
Table_exposure <- data.frame(
  ...
  "Area" = c(290,1005,936,290,834,92,725,3395,5743,1988,
           ...
           8490,46,227,81,440,1399,2619,1270,1122,634,1551),
...
n_gev_random <- function(n_mu, scale, shape) {
  ...
  out = NULL
  for (i in 1:n) {
    out[i] = rgev(1, loc = n_mu[i], scale = scale, shape = shape)
  }
  return(out)
}
...
InundationDepth <- function(depth, Table_exposure) {
  ...
  out = Table_exposure %>%
    ...
    filter(inundationdepth > 0)
  return(out)
}

LossRate_lookup <- function(inundationdepth, 
                            objecttype,
                            Table_susceptibility) {
  # find the relative loss for the inundation depth
  #       based on object type filtering
  # inundationdepth: single-value vector
  #       the difference between flood water table and 
  #       defense system return level (water table)
  # objecttype: single-value - land use type (commercial, residential)
  # Table_susceptibility: a data frame
  #######
  # return a relative loss corresponding to the object type and 
  #     inundation depth.
  dt = Table_susceptibility %>% filter(Object_type == objecttype)
  m = dim(dt)[1]
  for (i in 1:(m-1)) {
    if (inundationdepth >= dt$Inundation_depth[i] & 
        inundationdepth < dt$Inundation_depth[i+1]) {
      return(dt$Relative_loss[i])
      break
    }
  }
}

EAD <- function(Table_Exposure_Inund, 
                Inflation_co, 
                Table_susceptibility) {
  ...
  n = dim(Table_Exposure_Inund)[1]
  if (n == 0) {
    return(0)
  } else {
    lossrate = NULL
    for(i in 1:n) {
      lossrate[i] = LossRate_lookup(Table_Exposure_Inund$inundationdepth[i],
                                    Table_Exposure_Inund$Object_type[i],
                                 Table_susceptibility)
    }
    EAD_f = sum(lossrate / 100 * Table_Exposure_Inund$Area * Table_Exposure_Inund$Unitvalue)
    EAD_p = EAD_f / Inflation_co # compute the present value 
    return(EAD_p)
  }
}
EAD_years <- function(samples,
                      QH_paras,
                      H_defense,
                      Inflation_rate,
                      Table_exposure,
  ...
  tabPanel('Time-varying FR',
           tabsetPanel(
                 ...
                 sidebarLayout(
                   sidebarPanel(
                     selectInput(
                       ...
                       choices = c('Q-T relationship', 'Exposure', 'susceptibility'),
                       selected = 'Q-T relationship'
                     )
                   ),
                   mainPanel(
                     fluidRow(
                       column(6, DT::dataTableOutput("Data_show_table",
                                                     width = '90%',
                                                     height = '50%')),
                       column(6, plotOutput('Data_show_chart', 
             ...
             tabPanel(
               ...
               helpText('Note: Example data (historical river discharge observations) 
                        should be imported in "Sampling method" section', 
                        style = 'color:red'),
               fluidPage(
                 sidebarLayout(
                   sidebarPanel(
                     h4('Experiment setup'),
                     fluidRow(column(6,numericInput('TVFR_para_years', 'Simulation years:',
                                                    value = 50, min = 10, step = 5)),
                              column(6, numericInput('TVFR_para_runs', 'Number of runs:',
                   ...
                   mainPanel(
                     ...
                     fluidRow(
                       ...
                       column(6, br(), verbatimTextOutput('TVFR_EAD_sum'))
                     )
                   )
                 )
               )
...
server <- function(input, output) {
  ...
  AMS <- reactive({
    ...
    for (i in 1:n) {
      ...
      ams_date[i] = as.character(strftime(df1$date[which.max(df1$value)], format = "%d.%m.%Y"))
    }
    data.frame(
      'date' = ams_date, 
      'year' = years,
  ...
  output$plot_Maxima_daily <- renderPlot({
    ...
    ams = AMS()
    df$date = as.Date(df$date, format = "%d.%m.%Y")
    ams$date = as.Date(ams$date, format = "%d.%m.%Y")
    plot(df$date, df$value, type = "l", xlab='date', ylab='RV')
  ...
  output$plot_POT_daily <- renderPlot({
    ...
    df = Dataset()
    df$date = as.Date(df$date, format = "%d.%m.%Y")
    df_pot = df %>% filter(value >= input$threshold)
  ...
  output$PM_pp_chart <- renderPlot({
    if (input$Para_methods == 'Method of L-Moments' &
      ...
      req(input$Dataset)
      GEV_mle = GEV_mle()
      plot.fevd(GEV_mle, type = "probprob",
  ...
  # -- flood prequency analysis and GEV model fitting
  ...
  TVFR_df_sampling <- reactive({
    ...
    mu = GEV_mle$location
    sigma = GEV_mle$scale
    lambda = GEV_mle$shape
    CR_mu = input$TVFR_para_mu
    years = input$TVFR_para_years
    runs = input$TVFR_para_runs
    index = seq(1, years)
    seq_mu = mu + (index-1) * CR_mu /100 * mu
    
    sample_value = n_gev_random(seq_mu, scale = sigma, shape = lambda)
    if (runs == 1) { 
      # only one run
      sample_value
    } else {
  ...
  Table_exposure_TV <- reactive({
    Table_exposure %>% mutate(Area = Area * (1+input$TVFR_para_exposure))
  })
  
  Table_susceptibility_TV <- reactive({
    Table_susceptibility %>% 
      mutate(
        Relative_loss = Relative_loss * (1 + input$TVFR_para_susceptibility)
      )
  })
  
  # 
  # TVFR_df_exposure <- reactive({
  #   req(input$Dataset)
  #   df = TVFR_df_sampling()
  #   threshold = TVFR_df_threshold()
  # })
  ...
  TVFR_EAD <- reactive({
    ...
    if (n_runs ==1 ) {
      EAD_out = EAD_years(df,
                          QH_paras = c(input$TVFR_para_QT_a, input$TVFR_para_QT_b),
                          H_defense = threshold$return_level_H,
                          Inflation_rate = input$TVFR_para_inflation,
                          Table_exposure = Table_exposure_TV(),
                          Table_susceptibility = Table_susceptibility_TV()
      )
      EAD_out
    } else {
      ncols = dim(df)[2]
